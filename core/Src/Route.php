  ...
  public function start(): void
{
   ...
   // Убираем GET-параметры (?id=1)
   if (false !== $pos = strpos($uri, '?')) {
       $uri = substr($uri, 0, $pos);
   }
   $uri = rawurldecode($uri);

   // Автоматическое определение префикса: 
   // если URI начинается с префикса, отрезаем его, иначе оставляем как есть
   if (!empty($this->prefix) && strpos($uri, $this->prefix) === 0) {
       $uri = substr($uri, strlen($this->prefix));
   }

   // Гарантируем, что путь начинается со слэша и не пустой
   if (empty($uri)) {
       $uri = '/';
   }
   ...
   switch ($routeInfo[0]) {
       case Dispatcher::NOT_FOUND:
           // Для отладки: раскомментируйте строку ниже, если ошибка останется
           // die("Router looking for: '$uri' with method '$httpMethod'");
